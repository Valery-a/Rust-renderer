...
pub struct SkeletalAnimation {
    ...
    pub frames: Arc<Vec<BTreeMap<usize, SkeletalKeyframe>>>,
}
...
impl SkeletalBone {
    fn get_two_closest_keyframes_and_interpolate(
        &self,
        animations: &Vec<(Arc<SkeletalAnimation>, f64)>
    ) -> Mat4 {
        let mut min = (None, None, None);
        let mut max = (None, None, None);

        for (animation, weight) in animations {
            let approx_frame = (
                (animation.time / animation.max_time) *
                (animation.framecount as f32)
            ).floor() as usize;
                ...
                if let Some(keyframe) = animation.frames[frame].get(&self.index) {
                    if keyframe.time > animation.time {
                        if let Some(translate) = keyframe.translate {
                            ...
                            let new_mid =
                                prev_max + multiply_vec3_by_f64(translate - prev_max, *weight);
                            max.0 = Some(new_mid);
                        }
                        if let Some(rotate) = keyframe.rotate {
                            let prev_max = to_q64(max.1.unwrap_or(rotate));
                            let new_mid = from_q64(
                                interpolate_quaternion(prev_max, to_q64(rotate), *weight)
                            );
                            max.1 = Some(new_mid);
                        }
                        if let Some(scale) = keyframe.scale {
                            ...
                            let new_mid =
                                prev_max + multiply_vec3_by_f64(scale - prev_max, *weight);
                            max.2 = Some(new_mid);
                        }
                ...
                if let Some(keyframe) = animation.frames[frame].get(&self.index) {
                    if keyframe.time < animation.time {
                        if let Some(translate) = keyframe.translate {
                            ...
                            let new_mid =
                                prev_min + multiply_vec3_by_f64(translate - prev_min, *weight);
                            min.0 = Some(new_mid);
                        }
                        if let Some(scale) = keyframe.scale {
                            let prev_min = min.2.unwrap_or(scale);
                            let new_mid =
                                prev_min + multiply_vec3_by_f64(scale - prev_min, *weight);
                            min.2 = Some(new_mid);
                        }
            ...
            if
                (min.0.is_none() && min.1.is_none() && min.2.is_none()) ||
                (max.0.is_none() && max.1.is_none() && max.2.is_none())
            {
                if
                    let Some((i, _)) = animation.frames
                        .iter()
                        .filter(|f| f.contains_key(&self.index))
                        .enumerate()
                        .next()
                {
...
impl SkeletalAnimations {
    pub fn load_skeleton_stuff(
        skin: &gltf::Skin,
        _mesh: &gltf::Mesh,
        animations: gltf::iter::Animations,
        buffers: &[gltf::buffer::Data]
    ) -> Result<SkeletalAnimations, SkeletalAnimationError> {
        ...
        for animation in animations {
            let mut keyframes: HashMap<usize, BTreeMap<usize, SkeletalKeyframe>> = HashMap::new();
            let mut highest_time = 0.0;
            for channel in animation.channels() {
                let reader = channel.reader(|buffer| Some(&buffers[buffer.index()]));
                let times = reader.read_inputs().unwrap();
                let matrices = reader.read_outputs().unwrap();
                    ...
                    match &matrices {
                        ReadOutputs::Translations(translations) => {
                            ...
                            if !keyframes[&i].contains_key(&bone) {
                                ...
                                keyframes.get_mut(&i).unwrap().get_mut(&bone).unwrap().translate =
                                    Some(Vec3::new(translation[0], translation[1], translation[2]));
                            }
                        }
                        ReadOutputs::Rotations(rotations) => {
                            ...
                            if !keyframes[&i].contains_key(&bone) {
                                ...
                                keyframes.get_mut(&i).unwrap().get_mut(&bone).unwrap().rotate =
                                    Some(quat);
                            }
                        }
                        ReadOutputs::Scales(scales) => {
                            ...
                            if !keyframes[&i].contains_key(&bone) {
                                ...
                                keyframes.get_mut(&i).unwrap().get_mut(&bone).unwrap().scale = Some(
                                    Vec3::new(scale[0], scale[1], scale[2])
                                );
                            }
                        }
                        ReadOutputs::MorphTargetWeights(_err) => {
                            debug!("morph target weights");
                        }
                    }
                }
            }
            let mut keyframes_ordered: Vec<BTreeMap<usize, SkeletalKeyframe>> = keyframes
                .values()
                .cloned()
                .collect();
            keyframes_ordered.sort_by(|a, b|
                a
                    .iter()
                    .next()
                    .unwrap()
                    .1.time.partial_cmp(&b.iter().next().unwrap().1.time)
                    .unwrap()
            );
    ...
    pub fn apply_poses(
        ...
        for child in &bone.children {
            self.apply_poses(*child, pose, animations);
        }
