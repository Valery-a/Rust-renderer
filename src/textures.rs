...
impl Texture {
    pub fn load_texture(
        ...
        if let Ok(texture) = texture {
            ...
            error!("failed to load the texture: {}", name);
    ...
    pub fn new_from_name(name: &str) -> Result<Texture, TextureError> {
        ...
        let diffuse_data = load_image(diffuse_file_name.as_str()).map_err(|e|
            TextureError::LoadingError(e.clone())
        )?;

        {
            ...
            unsafe {
                ...
                TexImage2D(
                    ...
                    0,
                    SRGB_ALPHA as i32,
                    diffuse_data.dimensions.0 as i32,
                    diffuse_data.dimensions.1 as i32,
                    0,
                    RGBA,
                    UNSIGNED_BYTE,
                    diffuse_data.data.as_ptr() as *const GLvoid
                );
                ...
                let normal_data = load_image(normal_file_name.as_str()).map_err(|e|
                    TextureError::LoadingError(e.clone())
                )?;
                let metallic_data = load_image(metallic_file_name.as_str()).map_err(|e|
                    TextureError::LoadingError(e.clone())
                )?;
                let roughness_data = load_image(roughness_file_name.as_str()).map_err(|e|
                    TextureError::LoadingError(e.clone())
                )?;

                assert!(
                    diffuse_data.dimensions.0 == metallic_data.dimensions.0 &&
                        diffuse_data.dimensions.1 == metallic_data.dimensions.1
                );
                assert!(
                    diffuse_data.dimensions.0 == roughness_data.dimensions.0 &&
                        diffuse_data.dimensions.1 == roughness_data.dimensions.1
                );
                assert!(
                    diffuse_data.dimensions.0 == normal_data.dimensions.0 &&
                        diffuse_data.dimensions.1 == normal_data.dimensions.1
                );
                ...
                TexImage2D(
                    ...
                    0,
                    RGB as i32,
                    normal_data.dimensions.0 as i32,
                    normal_data.dimensions.1 as i32,
                    0,
                    RGBA,
                    UNSIGNED_BYTE,
                    normal_data.data.as_ptr() as *const GLvoid
                );
                TexParameteri(TEXTURE_2D, TEXTURE_MIN_FILTER, LINEAR_MIPMAP_LINEAR as i32);
                TexParameteri(TEXTURE_2D, TEXTURE_MAG_FILTER, LINEAR as i32);
                TexParameteri(TEXTURE_2D, TEXTURE_WRAP_S, REPEAT as i32);
                ...
                TexImage2D(
                    ...
                    0,
                    RGBA as i32,
                    metallic_data.dimensions.0 as i32,
                    metallic_data.dimensions.1 as i32,
                    0,
                    RGBA,
                    UNSIGNED_BYTE,
                    metallic_data.data.as_ptr() as *const GLvoid
                );
                TexParameteri(TEXTURE_2D, TEXTURE_MIN_FILTER, LINEAR_MIPMAP_LINEAR as i32);
                TexParameteri(TEXTURE_2D, TEXTURE_MAG_FILTER, LINEAR as i32);
                TexParameteri(TEXTURE_2D, TEXTURE_WRAP_S, REPEAT as i32);
                ...
                TexImage2D(
                    ...
                    0,
                    RGBA as i32,
                    roughness_data.dimensions.0 as i32,
                    roughness_data.dimensions.1 as i32,
                    0,
                    RGBA,
                    UNSIGNED_BYTE,
                    roughness_data.data.as_ptr() as *const GLvoid
                );
                TexParameteri(TEXTURE_2D, TEXTURE_MIN_FILTER, LINEAR_MIPMAP_LINEAR as i32);
                TexParameteri(TEXTURE_2D, TEXTURE_MAG_FILTER, LINEAR as i32);
                TexParameteri(TEXTURE_2D, TEXTURE_WRAP_S, REPEAT as i32);
    ...
    pub fn load_from_intermidiary(
        inter: Option<IntermidiaryTexture>
...
pub fn load_image(file_name: &str) -> Result<Image, String> {
    let img_data = std::io::BufReader::new(
        std::fs::File::open(file_name).map_err(|e| format!("Failed to load image: {}", e))?
    );
